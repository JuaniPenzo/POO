...
public class CuentaBancaria {
    ...
    public void depositar(double monto, String descripcion) {
        if (monto > 0) {
            ...
            Registro r = new Registro(movimientos.size() + 1, new Date(), "HABER", descripcion, monto, null, null, null);
            movimientos.add(r);
        }
    }
    ...
    public void depositar(double monto, String descripcion, Socio socio, Empleado empleado, Clase clase) {
        if (monto > 0) {
            saldo += monto;
            Registro r = new Registro(movimientos.size() + 1, new Date(), "HABER", descripcion, monto, socio, empleado, clase);
            movimientos.add(r);
        }
    }
    ...
    public boolean extraer(double monto, String descripcion, Socio socio, Empleado empleado, Clase clase) {
        if (monto > 0 && monto <= saldo) {
            saldo -= monto;
            Registro r = new Registro(movimientos.size() + 1, new Date(), "DEBE", descripcion, monto, socio, empleado, clase);
            movimientos.add(r);
            return true;
        }
        return false;
    ...
    public List<Registro> getMovimientosPorMes(int month, int year) {
        ...
        for (Registro r : movimientos) {
            ...
            if (y == year && (month == 0 || m == month)) result.add(r);
        }
        return result;
    }

    /**
     * Calcula totales (haber, debe, neto) para el mes/año indicado. month 1-12; month==0 -> todo el año
    ...
    public double[] calcularResumenMes(int month, int year) {
        ...
        for (Registro r : lista) {
            ...
            if (tipo.equalsIgnoreCase("HABER")) haber += monto;
            else if (tipo.equalsIgnoreCase("DEBE")) debe += monto;
            // ignore other tipos for the account summary
        }
